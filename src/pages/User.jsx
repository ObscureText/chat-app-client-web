...
function User() {
    ...
    const [userData, setUserData] = useState({
        ...
        image: location.state.image ? (URL + location.state.image) : '',
    });
	...
	useEffect(() => {
		...
		socketRef.current.on('message', (msg) => {
			...
			if(msg.sender === currentReceiverRef.current._id) {
				socketRef.current.emit('changeMessageStatus', { checked: true, message: msg})
			}
		...
		// for updating receiverStatus to 'read' of all in a chat
		socketRef.current.on('updateMessageStatusAll', ({chatId, receiver}) => {
			dispatch({
				type: 'UPDATE_ALL_STATUS_READ',
				payload: {
					chatId,
					receiver
				}
    ...
    const onReceiverChange = (receiver) => {
		...
		// check if there is any unread message in the current chat
		const hasUnreadMessages = messages.some(
			message => message.chatID === chatId && message.receiver === userData._id && message.receiverStatus !== 'read'
		);
		if(hasUnreadMessages) {
			// update status of all messages to 'read'
			socketRef.current.emit('changeMessageStatusAll', ({chatId: chatId, receiver: userData._id, sender: receiver._id}))
		}
	...
	const onUpdateButtonClick = () => {
		...
		axios.post( URL + '/edit-user-profile', formData ).then((res) => {	
			if(res.data.updatedUserData && res.data.newToken) {
				// change profile pic of user in state
				setUserData((data) => ({
					...data,
					image: (res.data.updatedUserData.image) ? (URL + res.data.updatedUserData.image) : ''
				}))
    ...
    return (
        <div className="user-page-main">
			...
			<img 
				src={phoneView ? '/close-button.png' : '/down-arrow.png' } 
				alt="arr" 
				onClick={()=>{setPhoneView((val) => !val)}} 
            ...
            <div className={`users-list ${phoneView ? 'open-list' : ''}`}>
                <div className="current-user-list-tile" style={{ order: '-1'}} key={userData._id}>
                    ...
                    <div className="logout-button-div">
                        <img src="/logout.png" alt="logout" onClick={ _ => {navigate('/login', {replace: true}); localStorage.clear()}} />
                    </div>
                </div>
                {
					(usersList.length === 0) ? (
						...
						usersList.map((value) => {
							...
							if(value._id === userData._id) { return null }
								...
								return <div className="list-tile" onClick={()=>{setPhoneView(false)}} style={{backgroundColor: '#404040'}} key={value._id}>
									{
										(value.status === 'online') && (
